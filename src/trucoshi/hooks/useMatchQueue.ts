import { useCallback, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import {
  EClientEvent,
  EServerEvent,
  IJoinQueueOptions,
  IQueueMatchFound,
  IQueueStatus,
} from "trucoshi";
import { useToast } from "../../hooks/useToast";
import { useTrucoshi } from "./useTrucoshi";
import { ADMISSION_MAINTENANCE_DETAIL, ADMISSION_MAINTENANCE_MESSAGE } from "../admission";
import { useGameAdmission } from "./useGameAdmission";

export type MatchQueuePlayerCount = 0 | IJoinQueueOptions["maxPlayers"];
export type JoinMatchQueueOptions = Omit<IJoinQueueOptions, "maxPlayers"> & {
  maxPlayers: MatchQueuePlayerCount;
};

export const reduceQueueStatus = (
  current: IQueueStatus | null,
  nextStatus: IQueueStatus | null,
) => {
  if (!nextStatus) {
    return null;
  }

  return !current || current.requestId === nextStatus.requestId ? nextStatus : current;
};

const showAdmissionUnavailable = (toast: ReturnType<typeof useToast>) => {
  toast.warning(`${ADMISSION_MAINTENANCE_MESSAGE}. ${ADMISSION_MAINTENANCE_DETAIL}`, {
    key: "game-admission-draining",
    preventDuplicate: true,
  });
};

export const useMatchQueue = () => {
  const [
    { isConnected, isQueueing, queueStatus: status },
    { setQueueing, setQueueStatus, setQueueReplayOptions },
    socket,
  ] = useTrucoshi();
  const toast = useToast();
  const navigate = useNavigate();
  const { canStartNewGames, reportAdmissionError } = useGameAdmission();

  const leaveQueue = useCallback(() => {
    if (!isQueueing) {
      setQueueStatus(null);
      setQueueing(false);
      return;
    }

    socket.emit(EClientEvent.LEAVE_QUEUE, ({ error }) => {
      if (error) {
        toast.error(error.message || "No se pudo salir de la cola");
      }
    });
    setQueueStatus(null);
    setQueueing(false);
  }, [isQueueing, setQueueStatus, setQueueing, socket, toast]);

  const joinQueue = useCallback(
    (options: JoinMatchQueueOptions) =>
      new Promise<boolean>((resolve) => {
        if (!canStartNewGames) {
          showAdmissionUnavailable(toast);
          resolve(false);
          return;
        }

        if (!isConnected) {
          toast.warning("Conectando con el servidor...");
          resolve(false);
          return;
        }

        setQueueing(true);
        setQueueReplayOptions(null);
        socket.emit(
          EClientEvent.JOIN_QUEUE,
          options as IJoinQueueOptions,
          ({ success, status: nextStatus, error }) => {
            if (!success) {
              setQueueing(false);
              setQueueStatus(null);
              if (reportAdmissionError(error)) {
                showAdmissionUnavailable(toast);
              } else {
                toast.error(error?.message || "No se pudo entrar a la cola");
              }
              resolve(false);
              return;
            }

            setQueueStatus(nextStatus || null);
            setQueueing(Boolean(nextStatus));
            resolve(true);
          },
        );
      }),
    [
      canStartNewGames,
      isConnected,
      reportAdmissionError,
      setQueueReplayOptions,
      setQueueStatus,
      setQueueing,
      socket,
      toast,
    ],
  );

  useEffect(() => {
    const handleQueueUpdate = (nextStatus: IQueueStatus | null) => {
      setQueueStatus((current) => reduceQueueStatus(current, nextStatus));
      setQueueing(Boolean(nextStatus));
    };

    const handleMatchFound = (match: IQueueMatchFound) => {
      setQueueing(false);
      setQueueStatus(null);
      setQueueReplayOptions(null);
      navigate(`/match/${match.matchSessionId}`);
    };

    socket.on(EServerEvent.QUEUE_UPDATE, handleQueueUpdate);
    socket.on(EServerEvent.QUEUE_MATCH_FOUND, handleMatchFound);

    return () => {
      socket.off(EServerEvent.QUEUE_UPDATE, handleQueueUpdate);
      socket.off(EServerEvent.QUEUE_MATCH_FOUND, handleMatchFound);
    };
  }, [navigate, setQueueReplayOptions, setQueueStatus, setQueueing, socket]);

  return {
    status,
    isQueueing,
    joinQueue,
    leaveQueue,
  };
};
