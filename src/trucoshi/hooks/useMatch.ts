...
const admissionDrainError = () => ({
  code: ADMISSION_DRAIN_ERROR_CODE,
  message: ADMISSION_DRAIN_ERROR_MESSAGE,
});

const showAdmissionUnavailable = (toast: ReturnType<typeof useToast>) => {
  toast.warning(`${ADMISSION_MAINTENANCE_MESSAGE}. ${ADMISSION_MAINTENANCE_DETAIL}`, {
    key: "game-admission-draining",
    preventDuplicate: true,
  });
};

const showNewGameActionError = (
  error: { message?: string } | null | undefined,
  reportAdmissionError: (error: unknown) => boolean,
  toast: ReturnType<typeof useToast>,
) => {
  if (reportAdmissionError(error)) {
    showAdmissionUnavailable(toast);
    return;
  }
...
export const useMatch = (
  ...
  const { canStartNewGames, reportAdmissionError } = useGameAdmission();
  const canStartNewGamesRef = useRef(canStartNewGames);
  canStartNewGamesRef.current = canStartNewGames;
  ...
  const createMatch = useCallback(
    (callback: ICallbackMatchUpdate) => {
      if (!canStartNewGamesRef.current) {
        showAdmissionUnavailable(toast);
        callback(admissionDrainError());
        return;
      }
      ...
      socket.emit(EClientEvent.CREATE_MATCH, ({ match, activeMatches, error }) => {
        ...
        if (error) {
          showNewGameActionError(error, reportAdmissionError, toast);
        }
        if (match) {
          setMatchState((prev) => ({
            ...prev,
    ...
    [socket, dispatch, reportAdmissionError, toast],
  );
  ...
  const addBot = useCallback(
    ...
    [reportAdmissionError, socket, toast],
  );
  ...
  const startMatch = useCallback(
    ...
    [matchId, matchState.match, reportAdmissionError, socket, toast],
  );
  ...
  const playAgain = useCallback(
    (callback: (newMatchSessionId?: string) => void) => {
      ...
      if (matchId && matchState.match) {
        socket.emit(EClientEvent.PLAY_AGAIN, matchId, ({ newMatchSessionId, error }) => {
          if (error) {
            showNewGameActionError(error, reportAdmissionError, toast);
          }
