...
describe("LobbySeatCard visual guard", () => {
  ...
  it("disables joining an empty seat while admission is draining", () => {
    const match = buildMatch({
      players: [buildPlayer({ key: "p1", teamIdx: 0, isMe: true, isOwner: true })],
    });
    renderSeat({
      match,
      slot: { key: "empty-1", teamIdx: 1, player: null },
      isAdmissionDraining: true,
    });

    expect(screen.getByRole("button", { name: /unirse/i })).toBeDisabled();
    expect(screen.getByRole("button", { name: /agregar bot/i })).toBeDisabled();
  });
});
