        ...
        <TrucoBoardLayout
          ...
          bottomContent={
            <Stack spacing={1} pb={{ xs: "3.7rem", sm: "3.4rem", md: "3.2rem" }}>
              {isAdmissionDraining ? <AdmissionNotice compact /> : null}
              {match.me?.isOwner ? (
                <LoadingButton
                  isLoading={isReadyLoading}
                  disabled={match.state !== EMatchState.READY || isAdmissionDraining}
                  variant="contained"
                  color="success"
...
export const Lobby = () => {
  ...
  const { isDraining: isAdmissionDraining } = useGameAdmission();
